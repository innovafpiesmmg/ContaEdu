...
async function addColumnIfNotExists(client: any, table: string, column: string, definition: string) {
...
async function createEnumIfNotExists(client: any, name: string, values: string[]) {
  const res = await client.query("SELECT 1 FROM pg_type WHERE typname=$1", [name]);
  if (res.rows.length === 0) {
    const vals = values.map(v => `'${v}'`).join(", ");
    await client.query(`CREATE TYPE ${name} AS ENUM(${vals})`);
    console.log(`  [auto-migrate] Created enum ${name}`);
  }
}
...
export async function runAutoMigrations() {
  ...
  try {
    ...
    await addColumnIfNotExists(client, "accounts", "description", "text");
    await addColumnIfNotExists(client, "accounts", "debit_when", "text");
    await addColumnIfNotExists(client, "accounts", "credit_when", "text");
    ...
    // Backfill course_exercises from existing exercises that have course_id
    await client.query(`
